...
|---|---|---|
| [`clean-dev`](skills/clean-dev/SKILL.md) | any coding task | Four eternal goals that also break ties between principles (local reasoning, contain complexity, readability, minimal blast radius); ten design commandments — SOLID, DRY (+ don't reinvent the wheel), defensible generalization from the bigger picture, cohesion-by-feature, deep modules, dependency inversion, root-vs-leaf stability — plus workflow gates: clarify before coding, think critically and own the project (challenge questionable requests, technical or product), explore for quality (and make sub-agents report it), the scout rule, and changing code without breaking untouched behavior |
